...
export const SHAPES = {
  ...
  Jobs: {
    ...
    toPg: async (f) => {
      ...
      if ("Sourcing" in f) out.sourcing = selectName(f.Sourcing) || "in_house";
  ...
  Employees: {
    ...
    toAirtable: async (row) => {
      ...
      return {
        ...
        fields: {
          ...
          "Home Lat": row.home_lat,
          "Home Lng": row.home_lng,
          "Home Set At": row.home_set_at,
          "Home Set By": row.home_set_by,
          Designation: row.designation,
    ...
    toPg: async (f) => {
      ...
      // Home geofence: both coords or neither (null clears it). Set-at/by are
      // stamped by the caller so the audit trail says who registered it.
      if ("Home Lat" in f || "Home Lng" in f) {
        const la = num(f["Home Lat"]);
        const ln = num(f["Home Lng"]);
        const ok = la != null && ln != null && Math.abs(la) <= 90 && Math.abs(ln) <= 180 && !(la === 0 && ln === 0);
        out.home_lat = ok ? la : null;
        out.home_lng = ok ? ln : null;
      }
      if ("Home Set At" in f) out.home_set_at = f["Home Set At"] || null;
      if ("Home Set By" in f) out.home_set_by = str(f["Home Set By"]);
      if ("Designation" in f) out.designation = str(f.Designation);
