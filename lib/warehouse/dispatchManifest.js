...
const INVOICE_SELECT =
  "id,dispatch_id,manifest_id,seq,invoice_no,eway_bill_no,client_id,customer_name," +
  "drop_city,invoice_value_inr,notes,created_at";

// Invoices/lines belong to EITHER a vehicle dispatch or a standalone manifest.
// ownerFilter turns a scope into the right PostgREST filter so every read/write
// below is owner-agnostic.
function ownerCol(scope) {
  return scope?.manifestId ? "manifest_id" : "dispatch_id";
}
function ownerId(scope) {
  return scope?.manifestId ?? scope?.dispatchId;
}

async function listInvoicesForOwner(scope) {
  const rows = await dbSelect("vehicle_dispatch_invoices", {
    select: INVOICE_SELECT,
    filter: { [ownerCol(scope)]: `eq.${ownerId(scope)}` },
...
export const listDispatchInvoices = (dispatchId) => listInvoicesForOwner({ dispatchId });
export const listManifestInvoices = (manifestId) => listInvoicesForOwner({ manifestId });
...
const LINE_SELECT =
  "id,dispatch_id,manifest_id,invoice_id,sr_no,master_product_id,item_id,sku,description,box_count," +
...
async function listLinesForOwner(scope) {
  const rows = await dbSelect("vehicle_dispatch_lines", {
    select: LINE_SELECT,
    filter: { [ownerCol(scope)]: `eq.${ownerId(scope)}` },
    order: "sr_no.asc,created_at.asc",
...
export const listManifestLines = (dispatchId) => listLinesForOwner({ dispatchId });
export const listGeneratorLines = (manifestId) => listLinesForOwner({ manifestId });
...
async function saveManifestForOwner(scope, { invoices = [], lines = [] } = {}) {
  const col = ownerCol(scope);
  const id = ownerId(scope);
  const invoiceRows = [];
  ...
  if (dupes.length) throw new Error(`Invoice ${invoiceRows.find((r) => r.invoice_no.toLowerCase() === dupes[0]).invoice_no} is listed twice on this vehicle`);
  ...
  await dbDelete("vehicle_dispatch_lines", col, id);
  await dbDelete("vehicle_dispatch_invoices", col, id);
  ...
  return {
    invoices: await listInvoicesForOwner(scope),
    lines: await listLinesForOwner(scope),
  };
}

export const saveManifest = (dispatchId, data) => saveManifestForOwner({ dispatchId }, data);
export const saveGeneratorManifest = (manifestId, data) =>
  saveManifestForOwner({ manifestId }, data);
