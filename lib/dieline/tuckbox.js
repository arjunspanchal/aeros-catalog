...
export function buildTuckboxDieline({ L, W, H, thickness, units = "mm" }) {
  ...
  const cx = Math.max(Lm / 2 + Hm + t + hi, lidW / 2 + hd); // centre x
  const segs = [];
  ...
  // ---------- base row with rolled sides ----------
  both((xm) => {
    line("crease", xm(Lm / 2), y3, xm(Lm / 2), y4); // base | side fold
    line("cut", xm(Lm / 2), y3, xm(Lm / 2 + Hm + t + hi), y3); // side assembly top edge
    line("cut", xm(Lm / 2), y4, xm(Lm / 2 + Hm + t + hi), y4); // bottom edge
    line("crease", xm(Lm / 2 + Hm), y3, xm(Lm / 2 + Hm), y4); // roll crease 1
    line("crease", xm(Lm / 2 + Hm + t), y3, xm(Lm / 2 + Hm + t), y4); // roll crease 2
    line("cut", xm(Lm / 2 + Hm + t + hi), y3, xm(Lm / 2 + Hm + t + hi), y4); // inner panel edge
    // dust-flap slits in the outer side wall, just inside the roll crease
    const sl = D / 5;
    line("cut", xm(Lm / 2 + Hm - 3), y3 + sl, xm(Lm / 2 + Hm - 3), y3 + 2 * sl);
    line("cut", xm(Lm / 2 + Hm - 3), y4 - 2 * sl, xm(Lm / 2 + Hm - 3), y4 - sl);
  });
