...
export function buildRig(styleId, { L, W, H, taper, cups }) {
  ...
  switch (styleId) {
    ...
    case "pizzabox":
    case "snackbox": {
      const lip = styleId === "pizzabox" ? 41 : 32;
      const base = root(W, L);
      const back = attach(base, "top", H, 90, 0, 0.4);
      attach(back, "top", H * 0.8, 160, 0.15, 0.5, 0.2);
    ...
    case "cakebox": {
      // wrap tube: lid | side | base | side (band height W)
      const lid = root(L, W);
      const s1 = attach(lid, "right", H, 90, 0.0, 0.35, 0.06);
      const base = attach(s1, "top", L, 90, 0.08, 0.43, 0.02);
      attach(base, "top", H, 90, 0.16, 0.51, 0.06);
      const F = H / 2 + 25;
      attach(base, "left", F, 90, 0.55, 0.85, 0.12);
      attach(base, "right", F, 90, 0.55, 0.85, 0.12);
      attach(lid, "left", F, 90, 0.75, 1, 0.05);
      attach(lid, "right", F, 90, 0.75, 1, 0.05);
      break;
    }
    case "burgerbox": {
    ...
    case "paperbag": {
      ...
      attach(back, "top", 0, 0, 1, 1, 0); // no-op keeps indices stable
      break;
    }
    case "cupcarrier": {
      if (+cups === 1) {
        // sling: handle | hole band | handle
        const band = L + 4;
        const b = root(W, band);
      ...
      // A-frame: wing (holes) | riser | handle | crest | handle | riser | wing
      const holeD = L, pitch = W, handleH = H;
      const BW = 2 * pitch;
      const wingD = holeD + 44;
      const riser = Math.max(28, holeD * 0.42);
      const wingA = root(BW, wingD);
      const rA = attach(wingA, "top", riser, 62, 0, 0.4, 0.08);
      const hA = attach(rA, "top", handleH, 28, 0.2, 0.6, 0.04);
      const hB = attach(hA, "top", handleH, 160, 0.45, 0.85, 0.04);
      const rB = attach(hB, "top", riser, 28, 0.6, 0.95, 0.08);
      attach(rB, "top", wingD, 62, 0.7, 1, 0.12);
      break;
    }
    default:
      return null;
...
export const RIGGED_STYLES = ["cakebox", "foodbox", "pizzabox", "snackbox", "burgerbox", "carton", "sleeve", "tuckbox", "tray", "envelope", "paperbag", "cupcarrier"];
