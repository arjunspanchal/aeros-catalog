...
export function buildRig(styleId, { L, W, H, taper, cups }) {
  ...
  switch (styleId) {
    ...
    case "cakebox": {
      ...
      attach(lid, "top", F, 90, 0.75, 1, 0.05);
      attach(lid, "bottom", F, 90, 0.75, 1, 0.05);
      // side dust flaps fold in first, then base flaps, then the lid closes
      attach(s1, "left", Fd, 90, 0.5, 0.78, 0.15);
      attach(s1, "right", Fd, 90, 0.5, 0.78, 0.15);
      attach(s2, "left", Fd, 90, 0.5, 0.78, 0.15);
      attach(s2, "right", Fd, 90, 0.5, 0.78, 0.15);
      // tuck flap on the lid's free edge, folds down last
      attach(lid, "left", Math.max(8, Math.min(22, H / 5)), 90, 0.85, 1, 0.1);
      break;
    }
    case "burgerbox": {
