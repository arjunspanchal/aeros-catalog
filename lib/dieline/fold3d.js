...
export function buildRig(styleId, { L, W, H, taper }) {
  ...
  switch (styleId) {
    ...
    case "burgerbox": {
      const base = root(W, L);
      const back = attach(base, "top", H, 90, 0, 0.4);
      attach(base, "bottom", H, 90, 0, 0.4);
      attach(base, "left", H, 90, 0.1, 0.5);
      attach(base, "right", H, 90, 0.1, 0.5);
      const lid = attach(back, "top", L, 90, 0.55, 0.95, 0.05);
      attach(lid, "top", H, 88, 0.85, 1, 0.18);
      break;
    }
    default:
      return null;
...
export const RIGGED_STYLES = ["cakebox", "foodbox", "pizzabox", "snackbox", "burgerbox", "carton", "sleeve", "tuckbox", "tray", "envelope"];
