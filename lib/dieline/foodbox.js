...
const T = 7; // wall taper per side (mm)
...
export function buildFoodboxDieline({ L, W, H, units = "mm" }) {
  ...
  const Wb = Wmm - 2 * T;
  if (Lb < 40 || Wb < 40) {
    return { segments: [], blank: null, warnings: [`Too small — internal top opening must be at least ${40 + 2 * T} mm each way (7 mm taper per side).`], valid: false };
  }
  ...
  for (const dir of [-1, +1]) {
    ...
    const k = { x: -1.02 * g, y: -5.82 * g }; // gusset attachment kink (scaled)
  ...
  for (const dir of [-1, +1]) {
    ...
    const yv = dir === -1 ? 0 : Lb; // notch tip y
