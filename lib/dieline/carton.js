...
export function buildCartonDieline({ L, W, H, cartonType = "rte", thickness, units = "mm" }) {
  ...
  // standard RTE/STE proportions: the tuck is a full-depth flap (~W, so it
  // reaches the panel bottom inside the box); dust flaps are ~W deep and
  // TAPER INWARD from the fold so they clear each other; the tuck's shoulders
  // sit 0.5-1 mm inside the panel edges + friction-lock nicks
  const dT = Math.max(12, Math.min(Wm - 2, 0.85 * Wm)); // tuck depth
  const rT = Math.min(8, dT / 3); // tuck corner radius
