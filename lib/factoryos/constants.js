...
// WFH workers punch against a per-employee registered home location instead
// (employees.home_lat/home_lng). Same radius idea; env-overridable
// (HOME_RADIUS_M). A WFH worker with no registered home cannot punch at all.
export const HOME_GEOFENCE_RADIUS_M = Number(process.env.HOME_RADIUS_M) || 300;

// Reject GPS fixes worse than this (metres). A 1 km+ "accuracy" is a
// cell-tower guess, not a location — it can't prove anyone is anywhere.
export const MAX_PUNCH_ACCURACY_M = Number(process.env.MAX_PUNCH_ACCURACY_M) || 500;
