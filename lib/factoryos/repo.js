...
function shapeJobFields(f) {
  ...
  if (f.sourcing !== undefined) out.Sourcing = f.sourcing;
...
// Recent self-punch coordinates for one employee (newest first). Used by the
// HR admin to register a WFH home geofence from where the person actually
// punches, instead of typing lat/lng by hand. Only rows with a check-in fix.
export async function recentPunchLocations(employeeId, limit = 10) {
  const emp = await findOne("employees", employeeId, "id");
  if (!emp) return [];
  const rows = await dbSelect("attendance", {
    select: "date,in_time,in_lat,in_lng,in_accuracy_m",
    filter: { employee_id: `eq.${emp.id}`, in_lat: "not.is.null", marked_by_name: "eq.self" },
    order: "date.desc",
    limit,
  });
  return rows.map((r) => ({
...
function shapeEmployeeFields(f) {
  ...
  // Home geofence (WFH). Pass homeLat/homeLng together; nulls clear it.
  if (f.homeLat !== undefined || f.homeLng !== undefined) {
    out["Home Lat"] = f.homeLat ?? null;
    out["Home Lng"] = f.homeLng ?? null;
    out["Home Set At"] = f.homeLat != null && f.homeLng != null ? new Date().toISOString() : null;
    out["Home Set By"] = f.homeSetBy ?? null;
  }
...
function normEmployee(row) {
  ...
  return {
    ...
    homeLat: typeof f["Home Lat"] === "number" ? f["Home Lat"] : null,
    homeLng: typeof f["Home Lng"] === "number" ? f["Home Lng"] : null,
    homeSetAt: f["Home Set At"] || null,
    homeSetBy: f["Home Set By"] || "",
...
// OT = hours worked past shift-end (default 19:00). Midnight-aware: when the
// check-out clock time is at/earlier than check-in, the shift crossed midnight
// (e.g. in 09:00, out 04:00), so we add 24h to the out time before measuring —
// otherwise an overnight OT shift would read as 0. Used by the worker punch
// clock, HR's manual entry, and (via stored ot_hours) payroll.
export function computeOtHours(inTime, outTime, shiftEndHm = "19:00") {
  const end = parseHm(shiftEndHm);
  let out = parseHm(outTime);
  const inn = parseHm(inTime);
  if (end == null || out == null) return 0;
  if (inn != null && out < inn) out += 24; // crossed midnight
...
// ---------- Hiring candidates ----------
// Prabhnoor's candidate pipeline (WorkIndia / Internshala / referrals). Direct
// Supabase; the only consumer of the `hiring_candidates` table.

function candNumOrNull(v) {
  if (v === "" || v == null) return null;
  const n = Number(v);
  return Number.isFinite(n) ? n : null;
}

function normCandidate(row) {
  return {
    id: row.id,
    name: row.name || "",
    phone: row.phone || "",
...
export async function listCandidates() {
  const rows = await dbSelect("hiring_candidates", { select: "*", order: "created_at.desc" });
  return rows.map(normCandidate);
}

export async function createCandidate(data) {
  if (!String(data.name || "").trim()) throw new Error("Candidate name is required");
  const row = await dbInsert(
    "hiring_candidates",
    {
      ...
      expected_salary: candNumOrNull(data.expectedSalary),
      location: data.location || null,
      notes: data.notes || null,
      link_url: data.linkUrl || null,
      stage: data.stage || "new",
      added_by_user_id: data.addedByUserId || null,
      added_by_name: data.addedByName || null,
    },
    { returning: "representation" },
  );
  return normCandidate(Array.isArray(row) ? row[0] : row);
}
...
export async function updateCandidate(id, patch) {
  ...
  if ("phone" in patch) out.phone = String(patch.phone || "").trim() || null;
  if ("source" in patch) out.source = patch.source || null;
  if ("role" in patch) out.role = patch.role || null;
  if ("experience" in patch) out.experience = patch.experience || null;
  if ("expectedSalary" in patch) out.expected_salary = candNumOrNull(patch.expectedSalary);
  if ("location" in patch) out.location = patch.location || null;
  ...
  const row = await dbUpdate("hiring_candidates", "id", id, out, { returning: "representation" });
  return normCandidate(Array.isArray(row) ? row[0] : row);
}

export async function deleteCandidate(id) {
  await dbDelete("hiring_candidates", "id", id);
  return { ok: true };
}
...
// Team-side inbox: every job with an UNREAD customer message — i.e. a
// customer-authored message newer than the team's last_read stamp. The mirror
// of customerInboxItems, flipped to author_role='customer' + team_last_read_at.
// Unlike the customer/vendor variants it takes no job list: internal staff
// (FE/FM/AM/Admin) share one inbox across every job, so we start from the
// (small) set of customer-authored messages and pull only their jobs. Powers
// /factoryos/inbox and the sidebar badge.
export async function teamInboxItems() {
  // Customer-authored messages are a small subset of the thread — filter on
  // them first, then look up only the jobs they belong to.
  const msgs = await dbSelect("job_messages", {
    select: "job_id,body,attachment_filename,created_at",
    filter: { author_role: "eq.customer" },
    order: "created_at.desc",
    limit: 5000,
  });
  ...
  const jobs = await dbSelect("jobs", {
    select: "id,airtable_id,j_number,item,brand,team_last_read_at",
  ...
  for (const j of jobs) {
    ...
    if (!m) continue;
    const unread =
      !j.team_last_read_at || new Date(m.created_at) > new Date(j.team_last_read_at);
    if (!unread) continue;
    let preview;
    if (m.body) preview = m.body.length > 140 ? m.body.slice(0, 140) + "…" : m.body;
    else if (m.attachment_filename) preview = `Attached: ${m.attachment_filename}`;
    else preview = "New message from customer";
    out.push({
      jobId: j.airtable_id,
      jNumber: j.j_number,
      jobItem: j.item,
...
// Count of jobs with an unread customer message — drives the sidebar badge.
// Thin wrapper over teamInboxItems (same two queries); kept separate so the
// layout reads intent at the call site. Never throws — a badge must not be
// able to break the page render, so callers still wrap it, but we guard here
// too for safety.
export async function teamUnreadCount() {
  try {
    ...
    console.error("teamUnreadCount failed:", e);
    return 0;
  }
}
...
// ---------- Delivery Plan ----------
// A job IS a PO line. These helpers read the delivery columns added in the
// 2026_06_27 migration (received_qty / delivery_status / delivery_remarks /
// order_rate) plus the job_dispatch_schedule child table — directly via
// dbSelect (snake_case), since normJob (Airtable-shim shape) doesn't surface
// them. Balance is derived (qty - received_qty); ageing from order_date.

const DELIVERY_OPEN_STATUSES = new Set(["unscheduled", "scheduled", "in_progress", "dispatched"]);
...
// Every delivery line the session can see (open + cancelled + done), enriched
// with its committed dispatch schedule. Scoping reuses listJobsForSession so
// role rules (AM client-scope, customer-scope) never drift. Callers filter to
// "open" (balance>0, not cancelled) for the board/matrix.
export async function listDeliveryLines(session) {
  const scoped = await listJobsForSession(session); // normJob shape, public ids
  if (!scoped.length) return [];

  // Delivery columns aren't in normJob (they bypass the shim) — pull them in
  // one shot and key by public id. The jobs table is small (~hundreds).
  const deliveryRows = await dbSelect("jobs", {
    select: "id,airtable_id,received_qty,delivery_status,delivery_remarks,order_rate",
    limit: 5000,
  });
  const pgByPublic = new Map();
  const deliveryByPublic = new Map();
  for (const r of deliveryRows) {
    const pub = r.airtable_id || r.id;
    pgByPublic.set(pub, r.id);
    deliveryByPublic.set(pub, r);
  }

  // Schedule rows for the scoped jobs only.
  const pgIds = scoped.map((j) => pgByPublic.get(j.id)).filter(Boolean);
  const scheduleByPg = new Map();
  if (pgIds.length) {
    const srows = await dbSelect("job_dispatch_schedule", {
      select: "id,job_id,dispatch_date,qty,status,note",
      filter: { job_id: `in.(${pgIds.join(",")})` },
      order: "dispatch_date.asc",
    });
    for (const s of srows) {
      if (!scheduleByPg.has(s.job_id)) scheduleByPg.set(s.job_id, []);
      scheduleByPg.get(s.job_id).push(s);
    }
  }

  // Client public-id -> name (jobs carry client public ids in clientIds).
  const clientRows = await dbSelect("clients", { select: "id,airtable_id,name", limit: 5000 });
  const clientNameByPublic = new Map(clientRows.map((c) => [c.airtable_id || c.id, c.name]));

  return scoped.map((j) => {
    const d = deliveryByPublic.get(j.id) || {};
    const pgId = pgByPublic.get(j.id);
    const ordered = typeof j.qty === "number" ? j.qty : 0;
    const received = typeof d.received_qty === "number" ? d.received_qty : 0;
    const balance = Math.max(0, ordered - received);
    const schedule = (scheduleByPg.get(pgId) || []).map((s) => ({
      id: s.id,
    ...
    const scheduledQty = schedule.reduce((t, s) => t + (s.qty || 0), 0);
    const rate = d.order_rate != null ? Number(d.order_rate) : null;
    const clientName =
      (j.clientIds || []).map((cid) => clientNameByPublic.get(cid)).find(Boolean) || j.brand || "";
    return {
      jobId: j.id,
      jNumber: j.jNumber,
      clientIds: j.clientIds,
      clientName,
      brand: j.brand,
      poNumber: j.poNumber,
      sku: j.masterSku || j.item || "",
      item: j.item || "",
      poDate: j.orderDate || null,
      orderRate: rate,
      ordered,
      received,
      balance,
      deliveryStatus: d.delivery_status || (scheduledQty > 0 ? "scheduled" : "unscheduled"),
      deliveryRemarks: d.delivery_remarks || "",
      stage: j.stage || "",
      daysPending: daysSince(j.orderDate),
      schedule,
      scheduledQty,
      openValue: rate != null ? balance * rate : null,
    };
  });
}

// True if a line should appear on the open board / feed the matrix.
export function isOpenDeliveryLine(line) {
  return line && line.balance > 0 && line.deliveryStatus !== "cancelled";
}

// Ageing summary over a set of lines (pure). Mirrors the spreadsheet buckets.
export function ageingBuckets(lines) {
  const defs = [
    { key: "critical", label: "Critical (>100d)", test: (d) => d != null && d > 100 },
    { key: "overdue", label: "Overdue (60–100d)", test: (d) => d != null && d > 60 && d <= 100 },
    { key: "watch", label: "Watch (30–60d)", test: (d) => d != null && d > 30 && d <= 60 },
    { key: "recent", label: "Recent (≤30d)", test: (d) => d == null || d <= 30 },
  ];
  return defs.map((b) => {
    const inB = lines.filter((l) => b.test(l.daysPending));
    return {
      key: b.key,
      label: b.label,
      lines: inB.length,
      balanceQty: inB.reduce((t, l) => t + (l.balance || 0), 0),
      openValue: inB.reduce((t, l) => t + (l.openValue || 0), 0),
    };
  });
}

// SKU × date aggregation of committed dispatch qty (pure). `dates` = ISO column
// keys. Drives the internal matrix and the customer date-wise table.
export function buildDispatchMatrix(lines, dates) {
  const cols = Array.isArray(dates) ? dates : [];
  const skuMap = new Map();
  for (const l of lines) {
    const sku = l.sku || "—";
    if (!skuMap.has(sku)) {
      skuMap.set(sku, { sku, byDate: Object.fromEntries(cols.map((d) => [d, 0])), total: 0 });
    }
    const entry = skuMap.get(sku);
    for (const s of l.schedule || []) {
      if (entry.byDate[s.dispatchDate] !== undefined) {
        entry.byDate[s.dispatchDate] += s.qty || 0;
        entry.total += s.qty || 0;
      }
    }
  }
  const rows = Array.from(skuMap.values()).filter((r) => r.total > 0);
  const dailyTotals = Object.fromEntries(
    cols.map((d) => [d, rows.reduce((t, r) => t + r.byDate[d], 0)]),
  );
  const grandTotal = rows.reduce((t, r) => t + r.total, 0);
  return { dates: cols, rows, dailyTotals, grandTotal };
}

// ---- Schedule CRUD (committed dispatch rows) ----
export async function getJobSchedule(jobId) {
  const pgId = await jobPgId(jobId);
  if (!pgId) return [];
  const rows = await dbSelect("job_dispatch_schedule", {
    select: "id,job_id,dispatch_date,qty,status,note,created_at",
    filter: { job_id: `eq.${pgId}` },
    order: "dispatch_date.asc",
  });
  return rows.map((s) => ({
...
export async function addScheduleRow(jobId, { dispatchDate, qty, note, email } = {}) {
  const pgId = await jobPgId(jobId);
  if (!pgId) throw new Error("Job not found");
  if (!dispatchDate) throw new Error("dispatchDate required");
  return dbInsert("job_dispatch_schedule", {
    job_id: pgId,
...
export async function updateScheduleRow(id, patch = {}) {
  const out = {};
  if (patch.dispatchDate !== undefined) out.dispatch_date = patch.dispatchDate;
  if (patch.qty !== undefined) out.qty = Math.max(0, Math.round(Number(patch.qty) || 0));
  if (patch.status !== undefined) out.status = patch.status;
  if (patch.note !== undefined) out.note = patch.note || null;
  if (!Object.keys(out).length) return null;
  return dbUpdate("job_dispatch_schedule", "id", id, out);
}

export async function deleteScheduleRow(id) {
  await dbDelete("job_dispatch_schedule", "id", id);
}

// Returns {jobPgId} for a schedule row — lets a route verify job access before
// editing/deleting a child row the caller only knows by schedule id.
export async function scheduleRowJobPgId(scheduleId) {
  const rows = await dbSelect("job_dispatch_schedule", {
    select: "job_id",
...
// ---- Delivery fields on a job (received / status / remarks / rate / qty) ----
// Written directly to PG (snake_case), bypassing the Airtable shim — these
// columns have no shim mapping. Internal-only callers (board + import).
export async function setJobDelivery(jobId, patch = {}) {
  const pgId = await jobPgId(jobId);
  if (!pgId) throw new Error("Job not found");
  const out = {};
  if (patch.receivedQty !== undefined)
    out.received_qty = Math.max(0, Math.round(Number(patch.receivedQty) || 0));
  if (patch.deliveryStatus !== undefined) out.delivery_status = patch.deliveryStatus || null;
  if (patch.deliveryRemarks !== undefined) out.delivery_remarks = patch.deliveryRemarks || null;
  if (patch.orderRate !== undefined)
    out.order_rate = patch.orderRate === null || patch.orderRate === "" ? null : Number(patch.orderRate);
  if (patch.qty !== undefined) out.qty = Math.max(0, Math.round(Number(patch.qty) || 0));
  if (patch.orderDate !== undefined) out.order_date = patch.orderDate || null;
  if (!Object.keys(out).length) return null;
  return dbUpdate("jobs", "id", pgId, out, { returning: "minimal" });
}

// Import open-PO lines for a client. For each row, match an existing job by
// (po_number, master_sku) and UPDATE its qty/received/rate/date; otherwise
// CREATE a minimal job (PO line) with a fresh J#. Returns a per-row result for
// the preview/confirm UI.
export async function importPoLines(clientPublicId, rows = [], { email } = {}) {
  const client = await findOne("clients", clientPublicId, "id,airtable_id,name");
  if (!client) throw new Error("Client not found");

  const existing = await dbSelect("jobs", {
    select: "id,airtable_id,po_number,master_sku,item",
    filter: { client_id: `eq.${client.id}` },
    limit: 5000,
  });
  const keyOf = (po, sku) =>
    `${String(po || "").trim().toLowerCase()}|${String(sku || "").trim().toLowerCase()}`;
  const byKey = new Map();
  for (const j of existing) byKey.set(keyOf(j.po_number, j.master_sku || j.item), j);
  ...
  for (const r of rows) {
    ...
    if (!poNumber && !sku) {
      results.push({ poNumber, sku, action: "skipped", reason: "missing PO# and SKU" });
      continue;
    }
    const ordered = Math.max(0, Math.round(Number(r.ordered) || 0));
    const received = Math.max(0, Math.round(Number(r.received) || 0));
    const rate = r.rate === "" || r.rate == null ? null : Number(r.rate);
    const poDate = r.poDate || null;
    try {
      const match = byKey.get(keyOf(poNumber, sku));
      if (match) {
        const patch = { qty: ordered, received_qty: received };
        if (rate != null && Number.isFinite(rate)) patch.order_rate = rate;
        if (poDate) patch.order_date = poDate;
        await dbUpdate("jobs", "id", match.id, patch, { returning: "minimal" });
        results.push({ poNumber, sku, action: "updated", jobId: match.airtable_id || match.id });
      } else {
        const jNumber = await getNextJobNumber();
        const created = await createJob({
          ...
          masterSku: sku,
          qty: ordered,
          orderDate: poDate || undefined,
          poNumber,
          stage: STAGES[0],
        });
        const extra = { received_qty: received };
        if (rate != null && Number.isFinite(rate)) extra.order_rate = rate;
        const pgId = await jobPgId(created.id);
        if (pgId) await dbUpdate("jobs", "id", pgId, extra, { returning: "minimal" });
        results.push({ poNumber, sku, action: "created", jobId: created.id });
      }
    } catch (e) {
      console.error("importPoLines row failed:", poNumber, sku, e?.message || e);
      results.push({ poNumber, sku, action: "error", reason: e?.message || "failed" });
    }
  }
  return results;
}
