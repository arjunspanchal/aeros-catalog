...
//      new Coating value (e.g. "PE 1-side" / "PE 2-side").
...
// it shows up in dropdowns. Matches the master's `Mill Coating` vocabulary so
// the whole system uses one label set ("PE 1-side" / "PE 2-side").
function coatingTag(coatingType) {
  if (coatingType === "PE 1-side") return "PE 1-side";
  if (coatingType === "PE 2-side") return "PE 2-side";
  // Back-compat for any pre-rename rows still carrying old labels.
  if (coatingType === "SSP") return "PE 1-side";
  if (coatingType === "DSP") return "PE 2-side";
  return coatingType || "";
}
