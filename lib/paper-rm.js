...
// Scoped Paper RM PAT (1.2). Read+write — `updateMasterPaper` is wired up
// behind the strict admin-only PATCH at /api/factoryos/master-papers/[id] for
// Base Rate / Discount edits. Falls back to legacy AIRTABLE_TOKEN during cutover.
function pickToken() {
  const v = process.env.AIRTABLE_PAT_PAPER_RM || process.env.AIRTABLE_TOKEN;
  if (!v) throw new Error("Missing env var: AIRTABLE_PAT_PAPER_RM (or legacy AIRTABLE_TOKEN)");
  return v;
}
...
function normMasterPaper(row) {
  ...
  return {
    ...
    supplier: f.Supplier || "",
    form: f.Form || "",
    // Mill-applied coating (e.g. ITC IndoBev ships PE-coated). Empty = uncoated. When
    // set, inventory receive flows should stamp the stock line's `Coating` so the PE
    // coating send-out page naturally skips it.
