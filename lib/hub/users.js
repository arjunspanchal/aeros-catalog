...
import { airtableList, escapeFormula, TABLES as ORDERS_TABLES } from "@/lib/orders/airtable";
...
export async function resolveEntitlements(email) {
  ...
  const calc = normalizeCalcRole(f["Calculator Role"]);
  ...
  return {
    ...
    ordersUserId: row.id,
    ordersClientIds: Array.isArray(f.Client) ? f.Client : [],
  };
}
