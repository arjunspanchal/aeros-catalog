...
// V-Bottom bags are always bundled into an inner poly bag (100 pcs/poly) before
// the outer carton — it's part of how they ship, so the poly cost is mandatory
// in the packing cost for this bag type. Both are admin-tunable per quote via
// `innerPack` / `innerPolyRate`; these are the defaults (₹/poly ≈ vendor rate).
export const INNER_POLY_COST_PER_POLY = 2.0;
export const INNER_POLY_DEFAULT_PACK = 100;
...
// Current slab structure (Arjun, 7-Aug-2026): TWO slabs — 50% @ ₹15/kg, 100% @ ₹25/kg.
// Legacy 10/30 keys kept so old saved quotes still recompute; new quotes use 50/100 only.
export const PRINTING_RATES = { 10: 7, 30: 10, 50: 15, 100: 25 };
...
export const OM_SHIVAAY_RATES = { "70": { 28: 52 }, "60": { 28: 55 }, "80": { 28: 48 }, "100": { 28: 48 } };
...
export function calculate(f) {
  ...
  // Inner poly bag — mandatory for V-Bottom (always poly-bagged in inners before
  // the outer carton). Cost per finished bag = poly rate ÷ pcs per poly.
  const innerPolyMandatory = f.bagType === "v_bottom_gusset";
  const innerPack = Math.max(1, Number(f.innerPack) || INNER_POLY_DEFAULT_PACK);
  const innerPolyRate =
    f.innerPolyRate !== undefined && f.innerPolyRate !== null && f.innerPolyRate !== ""
      ? Number(f.innerPolyRate)
      : INNER_POLY_COST_PER_POLY;
  const innerPolyCost = innerPolyMandatory ? innerPolyRate / innerPack : 0;
  ...
  const totalMfg =
    labourCost + wastageCost + cpCost + innerPolyCost + glueCost + paperCost +
  ...
  return {
    ...
    innerPolyCost: round4(innerPolyCost), innerPack, innerPolyRate,
...
// JK bleached (white) kraft — flat ₹/kg delivered across 80/100 gsm, 28 BF (Arjun, 7-Aug-2026).
export const JK_WHITE_RATE = 78;

export function lookupPaperRate({ paperType, mill, gsm, bf }) {
  const bfInt = parseInt(bf);
  if (mill === "JK" || (!mill && paperType === "Bleach Kraft White")) return JK_WHITE_RATE;
  ...
  if (paperType === "OGR Solidwrap 40 GSM") return 120; // Pudumjee Solidwrap, 40 gsm, flat
  if (paperType === "OGR Solidbag 40 GSM") return 113; // Pudumjee Solidbag, 40 gsm, flat
