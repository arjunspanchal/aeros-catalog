...
import { dbSelect, idFilterCol } from "../db/supabase.js";

export async function getMasterPaperRow(id) {
  if (!id) return null;
  // The catalogue surfaces each row's public id, which is the Airtable record id
  // (recXXX) for rows imported from Airtable and the Postgres uuid for native
  // rows. Match on airtable_id vs id accordingly — filtering the uuid column with
  // a recXXX id throws 22P02 (invalid uuid syntax).
