...
// Scoped Calculator PAT (1.2). Shared by /calculator/*, rate-cards, and the
// legacy aeros-paper-bag-calculator app (where it's read as AIRTABLE_API_KEY
// — leave that env name alone, it's pointing at the same PAT value). Falls
// back to legacy AIRTABLE_TOKEN during cutover.
function pickToken() {
  const v = process.env.AIRTABLE_PAT_CALCULATOR || process.env.AIRTABLE_TOKEN;
  if (!v) throw new Error("Missing env var: AIRTABLE_PAT_CALCULATOR (or legacy AIRTABLE_TOKEN)");
  return v;
}
...
export const TABLES = {
  ...
  ppQuotes: () => process.env.AIRTABLE_PP_QUOTES_TABLE || "PP Quotes",
