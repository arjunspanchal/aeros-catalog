...
// Selectable paper weights (grams per square metre) for the sidewall / outer
// fan. PE coating sits on top of this weight — the engine adds it separately.
export const GSM_INNER_OPTS = [220, 230, 240, 260, 280, 300, 320];
export const GSM_OUTER_OPTS = [200, 220, 230, 240, 260, 280, 300, 320];

// Bottom disc is fixed across the product line — 220 gsm board with a single
// PE coat. Customers + admins don't pick this.
export const BOTTOM_GSM = 220;
export const BOTTOM_COATING = "PE";

// Flexo colour counts the customer can pick. Everything above 4 is a custom
// quote.
export const COLOUR_OPTS = [1, 2, 3, 4];
...
export function customerFormToEngineInputs(cf) {
  ...
  const bottomGsm = BOTTOM_GSM;
  const weight = estimateCupWeightG({
    wallType: cf.wallType,
    size: cf.size,
    innerGsm: parseFloat(cf.inner?.gsm) || 0,
    outerGsm: parseFloat(cf.outer?.gsm) || 0,
    bottomGsm,
  });
  const overhead = customerOverheadPerCup(weight);
  ...
  return {
    ...
    swRate: String(cf.inner?.paperRate ?? CUSTOMER_DEFAULTS.innerPaperRate),
    ...
    btRate: String(cf.bottomPaperRate ?? CUSTOMER_DEFAULTS.bottomPaperRate),
    btCoating: BOTTOM_COATING,
    btCoatingRate: "",

    ofGSM: String(cf.outer?.gsm || ""),
    ofRate: String(cf.outer?.paperRate ?? CUSTOMER_DEFAULTS.outerPaperRate),
    ...
    // Single weight-scaled overhead bucket replaces flat conv/pack/glue.
    conv: String(overhead),
    pack: "0",
    glue: "0",
