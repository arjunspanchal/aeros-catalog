...
// Production hours per month: 12h shift × 25 days = 300 hrs. Admin can override.
export const MONTHLY_HOURS_DEFAULT = 300;

// Cups per minute by size. Single-wall baseline; double-wall and ripple run
// ~25% slower due to the outer-fan forming step. Admin form exposes these
// for tuning against the factory's actual production rates.
...
export const DW_SPEED_FACTOR = 0.75;
...
// Conversion cost per cup. Pure per-cup rate from fixed monthly overheads
// divided by the size-specific cups-produced-per-month.
export function computeConversionCostPerCup({
  ...
  const cupsPerMonth = monthlyHours * 60 * cpm;
...
export function customerFormToEngineInputs(cf) {
  ...
  const bottomGsm = 230;
  // Conversion: size + wall-type speed-aware formula from admin's monthly
  // overheads. Packing: fixed labour per cup + approximate poly/carton.
  // Glue: small flat rate. Matches the admin's conv/pack/glue split so the
  // client and admin numbers can't drift apart.
  const conv = computeConversionCostPerCup({ size: cf.size, wallType: cf.wallType });
  const clientPack = PACK_LABOUR_PER_CUP + 0.08; // labour + ~₹0.08/cup poly+carton avg
  const clientGlue = 0.05;
