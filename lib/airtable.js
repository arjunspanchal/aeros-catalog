...
// Scoped Clearance PAT (1.2). Falls back to the legacy single-base
// AIRTABLE_TOKEN during cutover; remove the fallback once Vercel envs are
// migrated and `git grep AIRTABLE_TOKEN lib/ scripts/` returns clean.
function pickToken() {
  const v = process.env.AIRTABLE_PAT_CLEARANCE || process.env.AIRTABLE_TOKEN;
  if (!v) {
    throw new Error(
      'Missing required env var AIRTABLE_PAT_CLEARANCE (or legacy AIRTABLE_TOKEN). ' +
      'Check your .env.local file or Vercel Environment Variables.'
    );
  }
  return v;
