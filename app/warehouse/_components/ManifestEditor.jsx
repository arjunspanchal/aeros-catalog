...
// Shared manifest editor — invoices, box types, live weight/CBM totals and the
// vehicle suggestion. Used by BOTH owners of a manifest:
//   * a vehicle dispatch (freight log, single customer, syncs box count/weight)
//   * the standalone manifest generator (lean, any mix of consignees)
// Everything owner-specific arrives as props (saveUrl, printUrl, the optional
// sync toggle and the "load last manifest" shortcut) so the editing behaviour
// itself can't drift between the two.
export default function ManifestEditor({
  ...
  clients = [],
  history = [],
  // Vehicle-dispatch only: the parent record, the sync-to-dispatch toggle, and
  // the "same as last time" shortcut. All optional.
  dispatch = null,
  lastManifest = null,
  ...
  const [syncDispatch, setSyncDispatch] = useState(!!showSyncToggle);
  ...
  function exportCsv() {
    ...
    a.href = url;
    a.download = `${dispatch?.dispatch_no || extraPayload?.manifestNo || "manifest"}.csv`;
  ...
  async function save({ vehicleSize } = {}) {
    ...
    try {
      ...
      setNote(
        [
          "Manifest saved.",
          showSyncToggle && syncDispatch && next.length
            ? "Dispatch box count and weight updated to match." : "",
        ...
        <div className="mt-4 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
          {suggestion.overflow ? (
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              {currentVehicleSize !== suggestion.vehicle.name && (
                <button
                  type="button"
