...
import { RUN_STATUSES } from "@/lib/orders/constants";
...
export default function RunsAdmin({ initialRuns, machines, jobs, currentUser }) {
  ...
  async function submit(e) {
    ...
    setErr("");
    setBusy(true);
    const res = await fetch("/api/orders/runs", {
      method: "POST",
        ...
        <div>
          ...
          {machines.length === 0 && (
            <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">
              No machines yet — <Link href="/orders/admin/machines" className="underline">add one first</Link>.
            </p>
          )}
        </div>
        <div>
