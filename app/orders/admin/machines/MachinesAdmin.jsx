...
import { MACHINE_TYPES, MACHINE_STATUSES } from "@/lib/orders/constants";
...
export default function MachinesAdmin({ initialMachines }) {
  ...
  async function submit(e) {
    ...
    setErr("");
    setBusy(true);
    const url = isEditing ? `/api/orders/machines/${editingId}` : "/api/orders/machines";
  ...
  async function removeMachine(m) {
    ...
    const res = await fetch(`/api/orders/machines/${m.id}`, { method: "DELETE" });
    if (!res.ok) {
