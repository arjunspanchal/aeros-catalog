...
export default function LoginForm() {
  ...
  async function requestOtp(e) {
    e.preventDefault();
    setErr(""); setBusy(true);
    const res = await fetch("/api/orders/auth/request-otp", {
      method: "POST",
  ...
  async function verifyOtp(e) {
    e.preventDefault();
    setErr(""); setBusy(true);
    const res = await fetch("/api/orders/auth/verify-otp", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function adminLogin(e) {
    e.preventDefault();
    setErr(""); setBusy(true);
    const res = await fetch("/api/orders/auth/admin", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 dark:bg-gray-950">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 w-full max-w-sm p-6 dark:bg-gray-900 dark:border-gray-800">
        ...
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">Aeros Order Tracking</h1>
        <p className="text-sm text-gray-500 mt-1 dark:text-gray-400">Sign in to continue</p>

        <div className="mt-5 flex gap-2 mb-5">
          <button
            onClick={() => { setMode("user"); setErr(""); setStage("enter"); }}
            className={`flex-1 py-2 rounded-lg text-sm font-medium ${mode === "user" ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300"}`}
        ...
        {mode === "user" && stage === "enter" && (
          <form onSubmit={requestOtp} className="space-y-3">
            <input
              type="email" className={inputCls} placeholder="you@example.com"
        ...
        {mode === "user" && stage === "otp" && (
          <form onSubmit={verifyOtp} className="space-y-3">
            ...
            <input
              ...
              className={`${inputCls} text-center tracking-widest font-mono`} placeholder="123456"
        ...
        {mode === "admin" && (
          <form onSubmit={adminLogin} className="space-y-3">
            <input
              type="password" className={inputCls} placeholder="Admin password"
